...
def ball_collision(ball_i):
    ...
    ball_radius = (x2-x1)/2.
    for other_i in range(ball_i+1, len(balls)):
        ...
        if test_collision(ball_center, ball_radius, other_center, other_radius):
            if ball_i not in collided or other_i not in collided:
                contact_angle = arctan(ball_center[0]-other_center[0],ball_center[1]-other_center[1])
                collide_balls(ball_i, other_i, contact_angle)
                collided.append(ball_i)
        else:
            while ball_i in collided:
                collided.remove(ball_i)
            while other_i in collided:
                collided.remove(other_i)
...
def collide_balls(ball_i, other_i, phi):
    ...
    print [[vx1i,vy1i],[vx2i,vy2i],phi]
    refvx, refvy = vx2i, vy2i
    vxi, vyi = vx1i-refvx, vy1i-refvy
    vi = math.hypot(vxi, vyi) 
    a1i = arctan(vx1i, vy1i)
    a2f = phi
    v2f = vi*math.sin(a2f-a1i)
    vx2f, vy2f = v2f*math.cos(a2f), v2f*math.sin(a2f)
    vx1f, vy1f = vxi-vx2f, vyi-vy2f
    vx2f += refvx
    vy2f += refvy
    vx1f = vx1i + vx2i - vx2f
    vy1f = vy1i + vy2i - vy2f
